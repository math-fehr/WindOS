...
int main() {
    ...
    volatile unsigned char* img = calloc(window_w*window_h*3,1);
    ...
    long int const iteration_max = 25;

    unsigned temp[iteration_max*2];
    ...
    for(unsigned x = 0; x< window_w; x++) {
        ...
        for(unsigned y = 0; y<window_h; y++) {
            ...
            do {
                ...
                z_i = 2*z_i*tmp + c_i;
                i++;
                temp[n_temp*2] = (z_r-x1)*zoom_x;
                temp[n_temp*2+1] = (z_i-y1)*zoom_y;
                n_temp++;
            } while(z_r*z_r+z_i*z_i < 4 && i < iteration_max);

//            if(i < iteration_max) {
                for(int j = 0; j<iteration_max; j++) {
                    if(temp[j*2] < window_w && temp[j*2+1] < window_h && img[(temp[j*2] + window_w*temp[j*2+1])*3] < 230) {
                        img[(temp[j*2] + window_w*temp[j*2+1])*3]+=1;
                        img[(temp[j*2] + window_w*temp[j*2+1])*3+1]+=1;
                        img[(temp[j*2] + window_w*temp[j*2+1])*3+2]+=1;
                    }
                }
//            }
        }
